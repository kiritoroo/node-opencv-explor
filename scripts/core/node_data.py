...
class NodeData:
  ...
  def start(self) -> None:
    ...
    if self.node_category == "base":
      self.node_detail = _factory_base.create_node(self.node_type)
      self.node = Node(self.node_name, _factory_base.color_bg)
    elif self.node_category == "filtering":
      self.node_detail = _factory_filtering.create_node(self.node_type)
      self.node = Node(self.node_name, _factory_filtering.color_bg)
    elif self.node_category == "morphology":
      self.node_detail = _factory_morphology.create_node(self.node_type)
