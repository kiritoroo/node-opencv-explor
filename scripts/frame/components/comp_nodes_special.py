...
class ComponentNodesSpecial:
  def __init__(self, ui_manager: pygame_gui.UIManager) -> None:
    self.ui_manager = ui_manager
    self._start()

  def __config_variables(self) -> None:
    self.nodes_special_info_path = ats.NODES_SPECIAL_INFO_PATH
    self.size_btn_node = pygame.Rect(0, 0, 180, 40)

    with open(self.nodes_special_info_path, 'r', encoding='utf-8') as _file:
      self.nodes_special_info_dict: dict = json.loads(_file.read())
      ...
      self.nodes_info_list = [
        ...
        "tooltip": v["tooltip"],
        "params": v["params"]}
        for v in self.nodes_special_info_dict.values()]
  ...
  def __config_ui_elements(self) -> None:
    ...
    for i in range(self.nodes_info_count):
      _ui_btn_node_base = pygame_gui.elements.UIButton(relative_rect=_rect,
              ...
              object_id=pygame_gui.core.ObjectID(object_id="@btn_node_special", class_id="#btn"))
