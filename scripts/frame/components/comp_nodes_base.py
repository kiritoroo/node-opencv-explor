...
class ComponentNodesBase:
  def __init__(self, ui_manager: pygame_gui.UIManager) -> None:
    self.ui_manager = ui_manager
    self._start()

  def __config_variables(self) -> None:
    self.nodes_base_info_path = ats.NODES_BASE_INFO_PATH
    self.size_btn_node = pygame.Rect(0, 0, 180, 40)

    with open(self.nodes_base_info_path, 'r', encoding='utf-8') as _file:
      self.nodes_base_info_dict: dict = json.loads(_file.read())
      ...
      self.nodes_info_list = [
        ...
        "tooltip": v["tooltip"],
        "params": v["params"]}
        for v in self.nodes_base_info_dict.values()]
